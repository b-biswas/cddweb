<?php
	...
	if (isset($_POST["register-user"]))
	{
	    ...
	    $first_name = htmlspecialchars(stripslashes(utf8_decode($_POST['firstName'])));
	    $last_name = htmlspecialchars(stripslashes(utf8_decode($_POST['lastName'])));
        ...
        if($valid == true)
        {
            ...
            {
                $errorMessage[] = 'Das Passwort muss mindestens 8 Zichen lang sein.';
            }
        ...
        if (empty($errorMessage))
        {
		    ...
		    else
		    {
				...
				if (!count($result) == 1)
				{
					$errorMessage[] = "Ihnen wird es nicht gestattet sich zu registrieren. Wenn Sie ein Student von der Doktorandenschule sind, können Sie sich mit die gleich Email-Adresse wie der Docktorandenschule registrieren. Falls Sie denken, es handelt sich um einen Fehler dann kontaktieren Sie uns bitte.";
				}
...
<h2 class="major">Registrieren</h2>
...
<form id="registration_form" action="" method="post">
    ...
    ?>
	<div class="field">
		<label for="login">E-mail Addresse :</label>
		<input type="text" name="userEmail" id="userEmail" required/>
	</div></br>
	<div class="field">
		<label for="login">Vorname :</label>
	...
	<div class="field">
		<label for="password">bestätigen Sie das Passwort :</label>
